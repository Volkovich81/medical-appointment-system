...
## Лабораторная работа №6: Concurrency

**Задание:**
- Реализовать асинхронную бизнес-операцию через `@Async`, которая:
  - возвращает `taskId`
  - позволяет проверять статус выполнения (`PENDING → PROCESSING → COMPLETED / FAILED`)
- Встроить асинхронность в существующую бизнес-логику (массовое добавление пациентов с валидацией)
- Продемонстрировать ошибку валидации и переход задачи в статус `FAILED`
- Реализовать потокобезопасный счётчик на `AtomicLong`
- Показать состояние гонки (race condition) при использовании обычного `long++`
- Провести нагрузочное тестирование в Apache JMeter:
  - 100 пользователей × 100 запросов = **10 000 запросов**
  - ошибка **0.00%**
- Сравнить потерю инкрементов на непотокобезопасном счётчике и точное значение на потокобезопасном
